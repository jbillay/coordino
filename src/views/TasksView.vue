<template>
  <AppLayout>
    <div class="tasks-view">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p class="text-gray-600 dark:text-gray-400">
            Manage your tasks with custom workflows
          </p>
        </div>
          ...
          <template #content>
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ taskStats.completionRate }}%</p>
              </div>
      ...
      <!-- Load More Button -->
      <div v-if="taskStore.hasMore && !taskStore.loading && displayedTasks.length > 0" class="flex justify-center mt-6">
        <Button
      ...
      <!-- Pagination Info -->
      <div v-if="taskStore.totalTasks > 0" class="text-center text-sm text-gray-500 dark:text-gray-400 mt-4">
...
const filteredTasks = computed(() => {
  return filterTasks(taskStore.tasks, filters.value)
})
...
const displayedTasks = computed(() => {
  return sortTasks(filteredTasks.value, sortBy.value)
})
...
const taskStats = computed(() => {
  return getTaskStats(taskStore.tasks)
})
...
const getEmptyMessage = computed(() => {
  ...
  const activeFilters = []
  if (filters.value.status) activeFilters.push('status')
  if (filters.value.category) activeFilters.push('category')
  if (filters.value.priority) activeFilters.push('priority')
