...
describe('useTimezone', () => {
  ...
  describe('formatWithTimezone (T060)', () => {
    it('should format time with timezone abbreviation', () => {
      ...
      // Should include time and timezone identifier
      // Timezone abbreviation (EST) or offset (UTC−5) may vary by environment
      expect(formatted).toContain('AM')
      expect(formatted).toContain('America/New_York')
      // Should include either timezone abbreviation or offset
      expect(formatted).toMatch(/EST|UTC[−-]5/)
    })
  })
  ...
  describe('calculateStatus (T061)', () => {
    ...
    it('should return critical for holidays (Priority 1)', () => {
      ...
      const result = calculateStatus(morningTime, DEFAULT_CONFIG, { name: 'Christmas' }, workDay)
    ...
    it('should return critical for non-working days (Priority 1)', () => {
      ...
      const result = calculateStatus(morningTime, DEFAULT_CONFIG, null, notWorkDay)

      expect(result.status).toBe('critical')
      expect(result.reason).toBe('Non-working day')
    })
    ...
    it('should return green for times within green hours (Priority 2)', () => {
      ...
      testCases.forEach(({ hour, minute }) => {
        ...
        const result = calculateStatus(time, DEFAULT_CONFIG, null, workDay)
        expect(result.status).toBe('green')
        expect(result.reason).toBe('Optimal working hours')
      })
    })
    ...
    it('should return orange for times in morning buffer (Priority 3)', () => {
      ...
      testCases.forEach(({ hour, minute }) => {
        ...
        expect(result.reason).toBe('Acceptable (early)')
      })
    })
    ...
    it('should return orange for times in evening buffer (Priority 3)', () => {
      ...
      testCases.forEach(({ hour, minute }) => {
        ...
        const result = calculateStatus(time, DEFAULT_CONFIG, null, workDay)
        expect(result.status).toBe('orange')
        expect(result.reason).toBe('Acceptable (late)')
      })
    })
    ...
    it('should respect custom config hours', () => {
      ...
      const result = calculateStatus(time, customConfig, null, workDay)
      expect(result.status).toBe('orange')

      // Test at 10:30 AM - should be green with custom config
      const greenTime = new Date(2025, 11, 12, 10, 30)
      const greenResult = calculateStatus(greenTime, customConfig, null, workDay)
    ...
    it('should prioritize holiday over time-based status', () => {
      ...
      const result = calculateStatus(greenHourTime, DEFAULT_CONFIG, { name: 'Christmas' }, workDay)
    ...
    it('should prioritize non-working day over time-based status', () => {
      ...
      const result = calculateStatus(greenHourTime, DEFAULT_CONFIG, null, notWorkDay)
